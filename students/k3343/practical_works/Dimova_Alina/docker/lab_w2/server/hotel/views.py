...
from hotel.models import Room, Client, Worker, Cleaning, Checkin, Floor, User, Otchet
from hotel.serializers import RoomSer, ClientSer, RoomDetailSer, CheckinSer, CleaningAddSer, AddClientSer, \
                        UserSerializer, WorkerSer, CleaningSer, AddCheckinSer, FloorSer, AddWorkerSer, ClientDetailSer,\
                        CleaningListSer, OtchetSerializer, AddOtchetSer, AddUserSer
...
class UserWorkerView(APIView):
    permission_classes = [permissions.AllowAny, ]

    def get(self, request, pk):
        users = User.objects.filter(is_worker=True)
    ...
    def post(self, request, pk):
        ...
        user.is_worker = request.data.get('is_worker')
        try:
            user.save()
            return Response({'status': 'Добавлено'})
        except:
...
class OtchetView(APIView):
    permission_classes = [permissions.IsAuthenticated, ]

    def get(self, request, pk):
        """ Просмотр всех отчетов одного работника """
        otchets = Otchet.objects.filter(worker=pk).order_by('date')
        serializer = OtchetSerializer(otchets, many=True)
        return Response({'data': serializer.data})

    def post(self, request, pk):
        """Добавление отчета от работника """
        otchet = AddOtchetSer(data=request.data)
        if otchet.is_valid():
            otchet.save()
            return Response({'status': 'Добавлено'})
        else:
